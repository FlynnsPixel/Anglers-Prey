...
public class Player {
	...
	public void init() {
		...
		player_light = Light.create(50, 0, 20, 2.5f, .4f, .5f, 1, 1);
	...
	public void update() {
		for (int i = 0; i < Light.lights.Count; ++i) {
			...
			//Light.lights[i].set_attribs(Light.lights[i].get_size() + Mathf.Cos(Time.timeSinceLevelLoad) / 40.0f, Light.lights[i].get_intensity());
		}
